    ...
    <div className="font-sans antialiased bg-white dark:bg-[#1E1E1E] min-h-screen flex flex-col transition-colors duration-500">
      ...
      <main className="flex-1 py-20 lg:py-32">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 w-full">
            ...
            {/* UPDATED GRID: 3 columns on all devices including mobile */}
            <div className="grid grid-cols-3 gap-3 md:gap-8 lg:gap-10">
                {visibleCategories.map((cat) => (
                    <div key={cat.id} className="group relative rounded-xl md:rounded-[2rem] overflow-hidden shadow-soft hover:shadow-premium hover:-translate-y-2 transition-all duration-700 border border-gray-100 dark:border-gray-800">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2 md:p-8 text-white">
                            <h3 className="text-[10px] sm:text-xs md:text-xl font-bold mb-0 md:mb-2 leading-tight">{cat.title}</h3>
                            <p className="hidden md:block text-xs md:text-sm text-gray-300 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 leading-relaxed">{cat.description}</p>
                        </div>
                    </div>
                ))}
            </div>
